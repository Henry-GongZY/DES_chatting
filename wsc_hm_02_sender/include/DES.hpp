...
unsigned short bit2us(datas_64 data,int start);
...
//    for(int i = (start+1)*16-1, j = 0;i >= start*16;i--,j++){
//        num += pow(2,j) * data.bits[i];
//    }
    for(int i = start*16 ; i <= start*16+15 ; i++){
        num += pow(2,i%16) * data.bits[i];
    }
    return num;
}